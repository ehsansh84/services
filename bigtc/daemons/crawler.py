...
def fetch(rss_item):
    try:
        ...
        if dup_count == 0 and new_count > 0:
            print(),
            log.color_print(color=Color.CYAN, text='ALL NEW!')
        elif new_count == 0:
            print(),
...
if exec_type == 'micro':
    ...
    rss_links = col_rss.find({'duration': {'$gte': 10}, 'active': 1, 'last_read': {'$lt':  d}}).sort('duration', 1)
    rss_count = col_rss.count({'duration': {'$gte': 10}, 'active': 1, 'last_read': {'$lt':  d}})
elif exec_type == 'huge':
...
try:
    ...
    for item in rss_links:
        ...
        try:
            ...
            log.color_print(text='Last Read: %s with link %s' % (item['last_read'].strftime("%Y-%m-%d %H:%M:%S"), item['link']), color=Color.YELLOW)
            if item['active'] == 1:
                print(i),
                fetch(item)
        except Exception, e:
